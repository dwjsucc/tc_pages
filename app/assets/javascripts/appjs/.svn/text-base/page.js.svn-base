...
museumapp.controller('museum_controller', function($scope, $http) {
  ...
  $scope.search_content;
	...
	//	$scope.type = response_type.data[0].id
    //
	//})
      ...
      callback();
		...
		$http({
			...
			$scope.response_img1 = response_img; //返回数据需要检查
			$scope.bigImg = response_img.data.big;
			$scope.bigName = response_img.data.big;
			$scope.smallImg = response_img.data.small;
			$scope.smallName = response_img.data.small;
		});
